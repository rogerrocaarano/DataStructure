...
void matrizDispersaVector::dimensionar(int n, int m) {
    df = n;
    dc = m;
}
...
void matrizDispersaVector::poner(int f, int c, matrizDispersaVector::DATA_TYPE valor) {
...
matrizDispersaVector::DATA_TYPE matrizDispersaVector::elemento(int f, int c) {
    if (f < 0 || f > df) {
...
void matrizDispersaVector::definir_valor_repetido(matrizDispersaVector::DATA_TYPE valor) {
...
int matrizDispersaVector::obtenerPosicion(int f, int c) {
    for (int i = 0; i < nt; i++) {
        if (Vf[i] == f && Vc[i] == c) {
            return i;
        }
...
void matrizDispersaVector::eliminar(int pos) {
    if (pos >= nt || pos < 0) {
...
void matrizDispersaVector::mostrar() {
    for (int i = 1; i <= df; i++) {
        for (int j = 1; j <= dc; j++) {
